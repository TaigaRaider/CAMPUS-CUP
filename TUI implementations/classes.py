"""
CAMPUS-CUP Data Models (TUI-adapted)

Cloned from root classes.py with the following modifications for TUI compatibility:
- Added `password` field to User class for authentication
- Removed interactive `Prompt.ask()` calls from League methods
- `populate_league()` now accepts a list of dicts instead of prompting
- `reschedule_match()` now accepts new_time as a direct parameter
- RuleSet methods now accept parameters directly
- Fixed Player.__repr__ bug (was printing user_name for both fields)
- Added __hash__ to Player for use in sets/dicts
- Removed auto-population call from League.__init__

All original class hierarchies and guard patterns are preserved.
"""
...
class User:
    def __init__(self, user_name: str, is_admin: bool, password: str = "password"):
...
class Player(User):
    def __init__(self, player_name: str, position: str, is_admin = False, password: str = "password"):
        super().__init__(player_name, is_admin, password)
    ...
    def __hash__(self) -> int:
        return hash((self.user_name, self.position))


class MatchOfficial(User):
    def __init__(self, match_official_name: str, password: str = "password"):
        super().__init__(match_official_name, is_admin=False, password=password)
...
class League:
    def __init__(self, league_name: str, league_size: int, sport: str, creator: User):
        ...
        # NOTE: Removed auto-population call for TUI compatibility.
        # Teams are added programmatically via the TUI forms.
    ...
    def populate_league(self, actor: User, teams_data: list[dict]):
        """Guarded method: Only Administrators can manage League states.

        TUI-adapted: accepts a list of dicts instead of interactive prompts.
        Each dict should have keys: 'team_name', 'captain_name', 'captain_position'
        """
        ...
        for team_data in teams_data:
            ...
            captain_position = team_data.get('captain_position', 'CF')
    ...
    def population_check(self, actor: User) -> int:
        ...
        if len(self.teams) < self.league_size:
            deficit = self.league_size - len(self.teams)
            return deficit
    ...
    class Match:
        ...
        def end_match(self, home_score: int, away_score: int):
            """TUI-adapted: accepts scores directly."""
        ...
        def reschedule_match(self, new_datetime: str, new_location: str = ""):
            """TUI-adapted: accepts new datetime and location directly."""
            if self.match_status != MatchStatus.CONCLUDED:
                self.match_datetime = new_datetime
                if new_location:
                    self.match_location = new_location
    ...
    class RuleSet:
        ...
        def update_ruleset(self, win: str, draw: str, loss: str, offence: str):
            """TUI-adapted: accepts values directly instead of prompting."""
            self.ruleSet["WIN"] = win
            self.ruleSet["DRAW"] = draw
            self.ruleSet["LOSS"] = loss
            self.ruleSet["OFFENCE"] = offence

        def validate_ruleset(self, confirmed: bool) -> bool:
            """TUI-adapted: accepts confirmation boolean directly.

            Returns True if ruleset was finalized, False if reverted.
            """
            if confirmed:
                self.temp_ruleSet = {}
                return True
